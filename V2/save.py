...
# (Ajoutez ici plus de termes si nécessaire pour les requêtes courantes)
...
# On suppose que nltk.download('stopwords') a été exécuté
try:
    # Utilisation des stopwords anglais car le corpus contient majoritairement de l'anglais (VO)
    ...
    try:
        with zipfile.ZipFile(path, "r") as z:
            for name in z.namelist():
                # Lit tous les fichiers de sous-titres (VO et VF)
...
class Moteur:
    ...
    def __init__(self, corpus: Dict[str, str]):
        ...
        # Vectoriseur rapide et efficace : n-grammes jusqu'à 3
        self.vectorizer = TfidfVectorizer(
            max_features=20000, 
            ngram_range=(1, 3), 
            sublinear_tf=True,
            stop_words=STOP_WORDS_FR_OR_EN # Utilisation des stopwords anglais par défaut
        )
        self.matrice = self.vectorizer.fit_transform(self.documents)
        print("INFO: Calcul TF-IDF terminé.")
        
        # Optimisation : Map vocabulaire/IDF (pour le scoring précis)
    ...
    def rechercher(self, requete: str, top_k: int) -> List[Tuple[str, float, Dict[str, float]]]:
        """ Recherche performante avec pondération hybride (TF-IDF + Poids IDF). """
        ...
        # 1. Score TF-IDF (Vectoriel rapide - Base de la pertinence)
        vec_requete = self.vectorizer.transform([requete_enrichie])
        scores_tfidf = cosine_similarity(vec_requete, self.matrice).flatten()

        # 2. Calcul des Bonus (Basés sur l'IDF)
        bonus_contexte_idf = np.zeros(self.nb_series)
        bonus_iconique = np.zeros(self.nb_series)
        
        # Poids ajustés : W_TFIDF (Base), W_IDF_CONTEXTE (Précision)
        W_TFIDF = 3.0           # Augmente le poids de la similarité statistique de base
        W_IDF_CONTEXTE = 15.0   # Augmente l'impact des mots rares trouvés
        W_ICONIQUE_BOOST = 5.0  # Boost fixe TRÈS AGRESSIF pour garantir les résultats ciblés
        ...
        for i in range(self.nb_series):
            ...
            # Calcul du score IDF de couverture (utilise les mots originaux et enrichis)
            for mot in mots_enrichis_set:
                if mot in texte:
                    mots_trouves += 1
                    # Le bonus est la somme des IDF (poids) des mots trouvés
                    idf_value = self.idf_map.get(mot, 1.0) 
                    total_idf_match += idf_value
            
            # Bonus Contextuel : Normalisé par le nombre de termes de la REQUÊTE ORIGINALE
            bonus_contexte_idf[i] = (total_idf_match / normalisation_mots)
            
            # 🚨 BOOST ICONIQUE / SÉMANTIQUE (Garantit les résultats clés) 🚨
            # On vérifie si la requête couvre au moins 75% des mots originaux ou un mot clé spécifique
            if mots_trouves >= (normalisation_mots * 0.75):
                # Lost / Crash avion île (island, plane, wreck, ile, avion, crash)
                if any(k in mots_requete_originaux for k in ['ile', 'avion', 'crash']) and 'lost' in serie_slug:
                ...
                if any(k in mots_requete_originaux for k in ['hopital', 'docteur']) and 'house' in serie_slug:
                     bonus_iconique[i] = W_ICONIQUE_BOOST
                
                # The Walking Dead / Zombie
                if any(k in mots_requete_originaux for k in ['zombie', 'dead']) and 'thewalkingdead' in serie_slug:
                     bonus_iconique[i] = W_ICONIQUE_BOOST
        ...
        for i in indices:
            ...
            # Seuil minimal pour éliminer le bruit total
            if score_final > 0.001: 
                details = {
                    'tfidf': scores_tfidf[i].item(),
                    # Score exact est le score IDF contextuel
                    'exact': bonus_contexte_idf[i].item(), 
                    'frequence': 0.0, # Non utilisé
